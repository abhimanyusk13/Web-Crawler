...
def iso_to_epoch(ts: str) -> int:
    # expects "YYYY-MM-DDTHH:MM:SS[.ffffff]Z"
...
async def run_indexer():
    ...
    # Query for docs updated after last_ts
    query: dict = {}
    if last_ts:
        query["updated"] = {"$gt": last_ts}
    cursor = col.find(query).sort("updated", 1)

    docs_to_index = []
    new_last_ts = last_ts
    async for doc in cursor:
        # transform Mongo doc → Typesense document
        ts_doc = {
            "id": str(doc["_id"]),
            "title": doc.get("title", ""),
            "body": doc.get("body", ""),
    ...
    # persist new_last_ts if we saw any docs
